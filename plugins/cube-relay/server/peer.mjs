#!/usr/bin/env node
// cube-relay peer — zero-dependency Claude Code MCP client for the cube-relay broker.
// Ported from the rulmo-relay client; defaults point at a cube-relay broker and it
// adds two robustness tweaks: a post-init tools/list_changed nudge (works around
// clients that race the initial tools/list during channel cold-start) and top-level
// crash handlers so a stray rejection can't kill the peer.
...
const BASE_URL = (process.env.CUBE_RELAY_BASE_URL || process.env.RELAY_BASE_URL || DEFAULT_BASE_URL).replace(/\/$/, '');
const WORKSPACE = process.env.CUBE_RELAY_WORKSPACE || process.env.RELAY_WORKSPACE || DEFAULT_WORKSPACE;
const TOKEN = process.env.CUBE_RELAY_TOKEN || process.env.RULMO_RELAY_TOKEN || process.env.RELAY_TOKEN || '';
const CWD = process.env.RELAY_WORKDIR || process.cwd();
const MACHINE = process.env.RELAY_MACHINE || hostname();
let peerSummary = process.env.CUBE_RELAY_PEER_NAME || process.env.RELAY_PEER_NAME || process.env.RELAY_PEER_SUMMARY || 'Cube Relay Claude Code peer';
let peerAlias = process.env.RELAY_PEER_ALIAS || peerSummary;
let peerGroups = [...new Set([...(process.env.CUBE_RELAY_PEER_GROUPS || '').split(','), ...(process.env.RELAY_PEER_GROUP || '').split(','), ...(process.env.RELAY_PEER_GROUPS || '').split(',')].map(s => s.trim()).filter(Boolean))];
...
const TOOLS = [
  ...
  { name:'complete_task', description:'Complete a relay task after handling a cube-relay channel message.', inputSchema:{type:'object',properties:{task_id:{type:'string'},summary:{type:'string'},status:{type:'string',enum:['completed','failed'],default:'completed'},artifacts:{type:'array',items:{type:'object'},default:[]}},required:['task_id','summary']} }
];

function log(msg) { console.error(`[cube-relay] ${msg}`); }
...
async function pollAndPushMessages() {
  if (!myId) {
    try { await ensureRegistered(); } catch (e) { log(`registration not ready for poll: ${e.message}`); return 0; }
  }
  let data;
  try {
  ...
  let pushed = 0;
  for (const m of (data.messages || [])) { pushChannelMessage(m, 'poll'); pushed++; }
  return pushed;
}
async function callTool(name, args={}) {
  if (name === 'relay_status') return { content:[{type:'text', text:JSON.stringify({peer_id:myId, registration_state:myId ? 'registered' : (registrationInFlight ? 'connecting' : 'not_registered'), last_register_error:lastRegisterError, base_url:BASE_URL, workspace:WORKSPACE, machine:MACHINE, peer_alias:peerAlias, group_name:peerGroups[0] || '', group_names:peerGroups, cwd:CWD, peer_name:peerSummary, summary:peerSummary}, null, 2)}] };
  ...
  if (name === 'list_peers') { await ensureRegistered(); const requestedGroup = String(args.group || '').trim(); const data = await requestJson('GET', '/v0/peers'); const peers = (data.peers||[]).filter(p => p.id !== myId).filter(p => sharesGroup(p.group_names || (p.group_name ? [p.group_name] : []), requestedGroup)).map(p => ({peer_id:p.id, address:p.peer_address||'', name:p.summary||'', group_name:p.group_name||'', group_names:p.group_names||[], cwd:p.cwd||'', branch:p.git_branch||'', status:p.status||'', age_seconds:p.age_seconds ?? null, last_seen:p.last_seen||''})); return {content:[{type:'text', text:JSON.stringify({group_names:peerGroups, peers}, null, 2)}]}; }
  if (name === 'send_to_peer_name') { await ensureRegistered(); const peer_name = String(args.peer_name||'').trim(); const message = String(args.message||'').trim(); const group_name = String(args.group||args.group_name||'').trim(); if (!peer_name || !message) throw new Error('peer_name and message are required'); const r = await requestJson('POST', `/v0/peers/${myId}/send`, {to_peer_name:peer_name, text:message, group_name, role_name:String(args.role_name||''), skill_name:String(args.skill_name||''), context_hash:String(args.context_hash||'')}); return {content:[{type:'text', text:`Sent to ${peer_name} (${r.to_peer_id}) as ${r.task_id}${r.group_name ? ` via group ${r.group_name}` : ''}. The reply will arrive as a cube-relay channel message.`}]}; }
...
function startPollingLoops() {
  ...
  heartbeatTimer = setInterval(() => { if (myId) requestJson('POST', `/v0/peers/${myId}/heartbeat`, {}).catch(e => { if (isBrokerNotFound(e)) { log(`peer ${myId} heartbeat returned 404; re-registering`); myId = null; startRegistration(); } }); }, HEARTBEAT_INTERVAL_MS);
...
function startRegistration() {
  ...
  registrationInFlight = register()
    .then(() => { lastRegisterError = null; return myId; })
    .catch(e => { lastRegisterError = e.message || String(e); log(`registration failed: ${lastRegisterError}`); throw e; })
    .finally(() => { registrationInFlight = null; });
  return registrationInFlight;
}
async function ensureRegistered() {
  if (myId) return myId;
  if (!registrationInFlight) startRegistration().catch(() => {});
  const detail = lastRegisterError ? ` Last error: ${lastRegisterError}` : '';
  throw new Error(`relay backend is still connecting; retry this tool after the peer registers.${detail}`);
}
...
async function handle(msg) {
  ...
  if (msg.method === 'notifications/initialized') {
    startPollingLoops();
    // Nudge clients to (re)fetch tools shortly after init — works around clients that
    // race the initial tools/list during dev-channel cold-start.
    setTimeout(() => notify('notifications/tools/list_changed', {}), 1000);
    return;
  }
...
// Resilience: a stray rejection/exception must not kill the peer process.
